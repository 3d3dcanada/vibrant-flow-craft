-- Phase 3G Gate 0 Launch Hardening
-- Date: 2026-01-14
-- Purpose: concurrency-safe RPCs, stricter maker write boundaries, and customer payload self-checks

-- 1) RLS + GRANT guardrails: remove maker update policy and block raw table writes
DROP POLICY IF EXISTS "Makers can update own assigned orders" ON public.maker_orders;

REVOKE INSERT, UPDATE, DELETE ON TABLE public.maker_orders FROM authenticated, anon;
REVOKE INSERT, UPDATE, DELETE ON TABLE public.maker_earnings FROM authenticated, anon;

-- 2) Concurrency-safe admin assignment with structured JSON errors
...
BEGIN
    ...
    -- Serialize assignment per order to avoid race conditions
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'EARNINGS_ALREADY_EXISTS';
    END IF;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        RETURN json_build_object(
            'success', false,
            'error', 'Order assignment failed'
        );
END;
$$;

-- 3) Structured errors + lifecycle guard for admin status updates
...
BEGIN
    ...
    IF p_new_status = 'delivered' THEN
        ...
        IF NOT FOUND THEN
            RETURN json_build_object(
                'success', false,
                'error', 'Order has no maker assignment'
            );
        END IF;
...
-- 4) Customer fulfillment RPC regression self-checks (comments only)
-- Example: status_history remains an array of objects
-- SELECT jsonb_typeof(
--   (customer_get_order_fulfillment('<order_id>')->'data'->'status_history')
-- ) AS status_history_type;
--
-- Example: ensure no actor identifiers leak in status_history
-- SELECT jsonb_path_exists(
--   customer_get_order_fulfillment('<order_id>')->'data'->'status_history',
--   '$[*] ? (@.changed_by != null || @.admin_id != null || @.maker_id != null || @.user_id != null)'
-- ) AS has_actor_ids;
--
-- Example: tracking_info only present when shipped/delivered
-- SELECT customer_get_order_fulfillment('<order_id>')->'data'->'tracking_info' AS tracking_info_snapshot;
